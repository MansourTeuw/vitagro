            ...
            <table class="table tabe-hover table-condensed" id="list">
                ...
                <tbody>
					...
					while($row= $qry->fetch_assoc()):
						...
						$tprog = $conn->query("SELECT * FROM tasks_list where project_id = {$row['pid']}")->num_rows;
		                $cprog = $conn->query("SELECT * FROM tasks_list where project_id = {$row['pid']} and status = 3")->num_rows;
						$prog = $tprog > 0 ? ($cprog/$tprog) * 100 : 0;
		                $prog = $prog > 0 ?  number_format($prog,2) : $prog;
		                $prod = $conn->query("SELECT * FROM users_productivity where project_id = {$row['pid']}")->num_rows;
