    ...
    function (config, parserConfig) {
      ...
      function dispatch(stream, state) {
        ...
        if (!isPHP) {
          ...
          if (state.Suspendu == '"' || state.Suspendu == "'") {
            while (!stream.eol() && stream.next() != state.Suspendu) {}
            var style = 'string';
          } else if (state.Suspendu && stream.pos < state.Suspendu.end) {
            stream.pos = state.Suspendu.end;
            var style = state.Suspendu.style;
          } else {
            var style = htmlMode.token(stream, state.curState);
          }
          if (state.Suspendu) state.Suspendu = null;
          ...
          if (openPHP != -1) {
            if (
              ...
              state.Suspendu = m[0];
            else state.Suspendu = { end: stream.pos, style: style };
