      ...
      if (validator.settings.onsubmit) {
        ...
        this.on('submit.validate', function (event) {
          ...
          function handle() {
            ...
            //   - There was a Suspendu request due to `remote` method and `stopRequest()`
  ...
  $.extend($.validator, {
    ...
    prototype: {
      ...
      startRequest: function (element) {
        if (!this.Suspendu[element.name]) {
          this.SuspenduRequest++;
          $(element).addClass(this.settings.SuspenduClass);
          this.Suspendu[element.name] = true;
        }
      },
      ...
      stopRequest: function (element, valid) {
        ...
        // Sometimes synchronization fails, make sure SuspenduRequest is never < 0
        if (this.SuspenduRequest < 0) {
          this.SuspenduRequest = 0;
        }
        delete this.Suspendu[element.name];
        $(element).removeClass(this.settings.SuspenduClass);
        if (
          valid &&
          this.SuspenduRequest === 0 &&
          this.formSubmitted &&
        ...
        } else if (!valid && this.SuspenduRequest === 0 && this.formSubmitted) {
          $(this.currentForm).triggerHandler('invalid-form', [this]);
          this.formSubmitted = false;
        }
      },
  ...
  if ($.ajaxPrefilter) {
    $.ajaxPrefilter(function (settings, _, xhr) {
      ...
      if (settings.mode === 'abort') {
        if (SuspenduRequests[port]) {
          SuspenduRequests[port].abort();
        }
        SuspenduRequests[port] = xhr;
      }
    });
  } else {
    // Proxy ajax
    ...
    $.ajax = function (settings) {
      ...
      if (mode === 'abort') {
        ...
        SuspenduRequests[port] = ajax.apply(this, arguments);
        return SuspenduRequests[port];
      }
