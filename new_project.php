                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select name="status" id="status" class="custom-select custom-select-sm">
                                <option value="0" <?php echo isset($status) && $status == 0 ? 'selected' : '' ?>>En
                                    cours...</option>
                                <option value="3" <?php echo isset($status) && $status == 3 ? 'selected' : '' ?>>En
                                    attente</option>
                                <option value="5" <?php echo isset($status) && $status == 5 ? 'selected' : '' ?>>Terminé
                                </option>
                            </select>
                        </div>
                    </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select class="form-control form-control-sm select2" name="manager_id">
                                <option></option>
                                <?php 
              	$managers = $conn->query("SELECT *,concat(firstname,' ',lastname) as name FROM userss where type = 2 order by concat(firstname,' ',lastname) asc ");
              	...
              	?>
                                <option value="<?php echo $row['land_id'] ?>"
                                    <?php echo isset($land_ids) && in_array($row['land_id'],explode(',',$land_ids)) ? "selected" : '' ?>>
                                    <?php echo ucwords($row['land_title']) ?></option>
                                <?php endwhile; ?>
                            </select>
                        </div>
                    </div>
...
$('#manage-project').submit(function(e) {
    ...
    $.ajax({
        ...
        success: function(resp) {
            if (resp == 1) {
                alert_toast('Activité ajouté avec succès!!!', "success");
                setTimeout(function() {
