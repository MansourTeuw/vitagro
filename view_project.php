...
$prod = $conn->query("SELECT * FROM users_productivity where project_id = {$id}")->num_rows;
...
$manager = $conn->query("SELECT *,concat(firstname,' ',lastname) as name FROM userss where id = $manager_id");
...
<div class="col-lg-12">
    ...
    <div class="row">
        ...
        <div class="col-md-4">
            <div class="card card-outline card-primary">
                <div class="card-header">
                    <span><b>Parcelles/Bassins Concernés:</b></span>
                    <div class="card-tools">
                        <!-- <button class="btn btn-primary bg-gradient-primary btn-sm" type="button" id="manage_team">Manage</button> -->
                    </div>
                </div>
                <div class="card-body">
                    <ul class="userss-list clearfix">
                        <?php 
						if(!empty($land_ids)):
							$lands = $conn->query("SELECT *,land_title as name FROM land where land_id in ($land_ids) order by land_title asc");
							while($row=$lands->fetch_assoc()):
						?>
                        <li>
                            <img src="assets/uploads/<?php echo $row['avatar'] ?>" alt="User Image">
                            <a class="land-list-name" href="javascript:void(0)"><?php echo ucwords($row['name']) ?></a>
                            <!-- <span class="userss-list-date">Today</span> -->
                        </li>
                        <?php 
							endwhile;
                ...
                <div class="card-header">
                    ...
                    <div class="card-tools">
                        <button class="btn btn-primary bg-gradient-primary btn-sm" type="button" id="new_task"><i
                                class="fa fa-plus"></i> Ajouter une Tâche</button>
                    </div>
                    <?php endif; ?>
                </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-condensed m-0 table-hover">
                            ...
                            <tbody>
                                <?php 
							$i = 1;
							$tasks = $conn->query("SELECT * FROM tasks_list where project_id = {$id} order by task asc");
                                    ...
                                    <td class="text-center">
                                        ...
                                        <div class="dropdown-menu" style="">
                                            <a class="dropdown-item view_task" href="javascript:void(0)"
                                                ...
                                                data-task="<?php echo $row['task'] ?>">Voir</a>
                                            <div class="dropdown-divider"></div>
                                            <?php if($_SESSION['login_type'] != 3): ?>
                                            <a class="dropdown-item edit_task" href="javascript:void(0)"
                                                data-id="<?php echo $row['id'] ?>"
                                                data-task="<?php echo $row['task'] ?>">Modifier</a>
                                            <div class="dropdown-divider"></div>
...
$('#new_task').click(function() {
    uni_modal("Nouvelle Tâche pour:: <?php echo ucwords($name) ?>", "manage_task.php?pid=<?php echo $id ?>",
        "mid-large")
})
